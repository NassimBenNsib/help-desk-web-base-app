...
function Login() {
  ...
  const form = useForm<z.infer<typeof AuthSchemasValidators.login>>({
    resolver: zodResolver(AuthSchemasValidators.login),
    defaultValues: {
      email: "",
      password: "",
    },
  ...
  async function onSubmit(values: z.infer<typeof AuthSchemasValidators.login>) {
    setIsLoading(true);
    AuthServices.login(values).then((data) => {
      // if (data.type === "success") form.reset();
      login(data.result, rememberMe);
      if (data.type === "error") {
        setIsLoading(false);
        setResponse(data);
      }
