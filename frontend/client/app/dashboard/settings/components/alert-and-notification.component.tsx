...
function AlertAndNotificationComponent() {
  const { notificationPreference, token, updateNotificationPreference } =
    useAccountStore();
  const [form, setForm] = useState({
    ...
    tickets: notificationPreference.tickets,
    usersActivity: notificationPreference.usersActivity,
    virtualAssistant: notificationPreference.virtualAssistant,
  });
  ...
  async function handleSubmit() {
    setIsLoading(true);
    UserSettingsServices.changeNotificationPreference(token, form)
      .then((data) => {
        console.log(data);
        setResponse(data);
        if (data.type === "success") updateNotificationPreference(form);
      })
      .finally(() => {
        setIsLoading(false);
      });
...
export { AlertAndNotificationComponent };
